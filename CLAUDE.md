...
- `25-attachments.md` — Reviewer **file attachments** on any annotation (line comment, image comment/region, note reply). Attach via a button or drag-drop; bytes stored under `<dataDir>/attachments/` with an `attachments` table; preview via OS **Quick Look** (`qlmanage -p` on macOS, default opener elsewhere — the local Node server shells out, like "reveal in file manager"); the markdown export lists each attachment's absolute path so the AI tool can read it. P1 shipped: data/API layer (generic over any annotation) + the chip UI on **line annotations** + Quick Look + export; follow-ups: the same UI on image-feedback/note-reply surfaces, paste-to-attach, chip thumbnails. Client UI: `src/client/annotations/attachments.tsx`
