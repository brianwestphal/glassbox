...
- **The morph no longer positionally adopts a protected node.** An element carrying `data-morph-skip` / `data-morph-skip-children` / `data-morph-preserve` is never reused as a positional stand-in for an unrelated template element; the template element is inserted fresh beside it, and a keyed match still morphs in place. Glassbox leans on all three markers (the diff content under `data-morph-skip`, server-rendered shells under `-children`, imperatively-injected nodes under `-preserve`), so this is a correctness fix in our favor — under kerf 2.x a conditional sibling reappearing at a protected node's position could have the widget swallow it, duplicate the widget, or leak a bound hole's text into it.
