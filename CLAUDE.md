...
- `src/ground-truth/manifest.ts` — Ground-truth comparison (doc 26 P1): `loadGroundTruthManifest(path)` reads/validates the JSON manifest and resolves each actual↔expected image pair (paths relative to the manifest dir, may live outside the repo)
