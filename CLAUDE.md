...
- `25-attachments.md` — Reviewer **file attachments** on any annotation (line comment, image comment/region, note reply). Attach via a button, drag-drop, or **paste**; bytes stored under `<dataDir>/attachments/` with an `attachments` table; image chips show a **thumbnail** and open a shared **lightbox** (`src/client/lightbox.tsx`); other types open via OS **Quick Look** (`qlmanage -p` on macOS, default opener elsewhere — the local Node server shells out, like "reveal in file manager"); the markdown export lists each attachment's absolute path so the AI tool can read it. Wired on line annotations, note replies, and image-feedback comments/regions. Also (GB-953) clicking an AI review-note **image artifact** opens the lightbox to drag a region that rides into the reply as a marked thumbnail (`src/components/reviewNoteRegionThumb.tsx`). Client UI: `src/client/annotations/attachments.tsx`
- `26-ground-truth-comparison.md` — **Design only.** A separate mode to compare an **actual** image against an **expected**/ground-truth image (design spec, reference render, or previous-actual baseline), even when the expected lives outside the repo. **Manifest-driven** actual→expected mapping; reuses the doc-24 comparison modes + doc-23 region marking + doc-18 arbitrary-path plumbing. v1 = single still images; a perceptual diff (filter identical / anti-aliasing-tolerant / difference score) is a fast-follow; sets/flows + capture pipeline (incl. domotion animated SVGs) are later. Phased follow-up tickets track implementation
