...
- `20-ai-review-notes.md` — AI-authored, line-anchored review notes (rationale/proof) emitted by the generating AI, stored as committed SARIF in `.pr-notes/`, rendered review-comment-style in the diff (P1–P5 shipped: the `.pr-notes/` SARIF format + the `glassbox note` producer CLI, the reader/render path with load-time re-anchoring + stale "outdated" badges, threaded human replies via `reply_to_note_id`, analysis-prompt + markdown-export folding, and the P4 artifact foundation — text/diagram-source inline blocks + image artifacts via `GET /api/review-notes/artifact` with sha-256 + Git LFS wiring; only live diagram rendering remains pending)
