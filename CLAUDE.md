...
- **Line coverage is a floor, not a ceiling — it measures the wrong thing for a whole class of bugs.** 100% line/branch/function/statement coverage proves every *line ran*; it says nothing about whether every documented *behavior*, or every *sequence* of behaviors, is actually *asserted*. A bug that lives in an untested interaction or state transition ships green under a 100% report, because the individual lines still get hit by isolated single-operation tests. So treat a high coverage number as a *trigger* for the feature/behavior audit below, never as a stopping point. The orthogonal **feature-coverage** axis (`npm run check:features`, `docs/testing/9-feature-coverage.md`) checks that each documented requirement has an *asserting* test; `tests/unit/conventions.test.ts` pins the structural invariants line coverage can't express.
- **Transition-matrix testing is mandatory for stateful modules.** For any module with modes / phases / a state machine / a cache-with-fallback / lifecycle transitions (e.g. review-mode branches, sort modes, the completion-modal / settings-dialog stage signals, the Apple-FM cache + fallback retry, the difftool accumulating session), do **not** stop at testing each operation from a clean initial state. Test the *transitions between states* — out-of-order, interleaved, repeated, empty-then-refill / refill-after-clear, and fallback-then-recovery. Untested transitions are the exact gap line coverage is structurally blind to; a stateful module at 100% line coverage with no transition tests is a *finding*, not a pass.
