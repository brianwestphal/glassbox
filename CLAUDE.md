...
- **Feature-coverage report** (`npm run check:features`, `scripts/check-features.ts`): the coverage axis orthogonal to line coverage. Extracts a requirement unit per documented behavior from `docs/[0-9]*.md` (both `**FR-N.M**`/`**NFR-N.M**` ids and older `### N.M` subsections) and flags any unit with no asserting test in the `docs/testing/feature-coverage.json` map — including stateful units that list no state transitions. Advisory by default; `-- --strict` exits non-zero on gaps. The map ships intentionally partial; see `docs/testing/9-feature-coverage.md`. Structural invariants line coverage misses live in `tests/unit/conventions.test.ts`.
