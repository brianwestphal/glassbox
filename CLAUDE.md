...
- `src/review-notes/` — AI-authored review notes (doc 20, P1): the `.pr-notes/` SARIF format (`sarif.ts`), the path-sharded on-disk store (`store.ts`), the `glassbox note` CLI (`cli.ts`; `add` / `update` / `remove` / `coalesce` / `instructions`), and the inbound AI-instructions contract (`instructions.ts`, printed by `glassbox note instructions`). Glassbox is the consumer of a tool-neutral, producer-written format.
