...
- `src/review-notes/` — AI-authored review notes (doc 20, P1): the `.pr-notes/` SARIF format (`sarif.ts`), the path-sharded on-disk store (`store.ts`), and the `glassbox note` CLI (`cli.ts`; `add` / `update` / `remove` / `coalesce`). Glassbox is the consumer of a tool-neutral, producer-written format.
