...
- Build artifacts per platform:
  - macOS: `.dmg` installer
  - Linux: `.AppImage`, `.deb`, and `.rpm` (Tauri `targets: "all"`; the `.rpm` is linked in the release Download summary as "Red Hat / Fedora")
  - Windows: `.msi` and `.exe`
- The build pipeline shall download Node.js v20 for the target platform and bundle it as a sidecar.
- The build shall detect a dev-mode sidecar stub (file size < 1MB) in a production build and **replace** it by re-downloading the real Node binary, so a stub can never ship (`scripts/build-sidecar.sh`).
...
- The server shall be built with tsup as an ESM bundle, with external dependencies (`@electric-sql/pglite`, `hono`, `@hono/node-server`, `@modelcontextprotocol/sdk`, `kerfjs`, `apple-fm`, plus `@preact/signals-core` — a transitive kerfjs dep kept external and copied into the sidecar).
...
- Release artifacts shall be created as **draft** GitHub Releases so the release stays hidden (off the public Releases page and the `latest` pointer) while assets upload mid-build, then be **automatically flipped to published** by a dedicated end-of-pipeline job once every bundle is uploaded (`publish-release` sets `draft: false, make_latest: 'true'` in `release-desktop.yml`; the beta track flips its prerelease draft likewise in `release-candidate.yml`). Publication is automated — there is no manual promotion step. The draft phase is an atomicity mechanism, not a human gate.
- Two workflows handle the tag-driven release pipeline:
  - `release-candidate.yml` — a **unified** workflow triggered by **both** `v*-rc.*` (release candidate) and `v*-beta.*` (beta) tags. The two tracks share every validation/publish phase and branch on `contains(github.ref, '-beta.')`:
    ...
    - The two **smoke** jobs (fresh install + upgrade-from-latest) install the just-published version via `scripts/release/install-published.sh`, which polls the registry until the version is listed (bounded, 15 min default — a publish once took six minutes to become visible), retries the install, and **asserts the installed version**. The assertion is the important part: without it the upgrade smoke once passed against the previously installed stable when propagation lagged, so a miss looked like a pass.
...
- The release validation phase shall include a **Rust gate** (`rust` job in `release-candidate.yml`) holding the Tauri desktop shell (`src-tauri/`) to the same fast-check bar as the TypeScript: `cargo fmt --check` (formatting), `cargo clippy --all-targets -- -D warnings` (lints as errors), and `cargo test` (unit tests). The job runs on Linux and installs the webkit2gtk dev headers because the `glassbox` crate links tauri/webkit even for `clippy`/`check`; it needs no client build because everything the crate compiles against (`loading/`, `capabilities/`, `Cargo.lock`) is committed — but it does first run `scripts/ensure-sidecar-stub.sh` to create the sidecar stub + a `server/` placeholder, without which `build.rs` panics. The gate runs alongside `lint` / `typecheck` / `test-unit` / `audit` and blocks every downstream publish job, so a formatting, clippy, or test failure on the Rust side fails the release exactly like a TypeScript lint failure. (Note: this is the only Rust *test/lint* gate — the other workflows that install Rust only *compile* the app via `tauri build`.)
- The release validation phase shall include a **database-migration gate** (`test-migration` job in `release-candidate.yml`) that performs a **real** PostgreSQL 17 → 18 data-directory migration against real PGlite engines (doc [9](9-data-storage.md) §9.1a). It is a release gate rather than a manual workflow because a regression here does not merely fail a feature — it meets an upgrading user with an unopenable database, and the default suite covers that path with mocks only. The job runs only `tests/integration/db/major-migration.test.ts` under `vitest.config.live.ts`; it needs **no** system tooling (unlike the live-render tests it does not provision a JRE or Chromium), only network to fetch the pinned old engine, so it costs a plain runner ~7s. Because the test is `describe.skipIf`-gated, the job asserts from the JSON reporter that tests actually **ran** and none were **pending** — otherwise a drifted env gate or config `include` would report green having executed nothing, the same silent false-negative `live-render.yml` guards against by asserting its tooling is present. The gate runs alongside `lint` / `typecheck` / `test-unit` / `audit` / `rust` and blocks every downstream publish job.
