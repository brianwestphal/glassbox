...
- The Apple Foundation Models helper, a native arm64 Mach-O bundled under Tauri `resources/**` (which Tauri does not sign, unlike its `externalBin` sidecars), shall be signed with the Developer ID + hardened runtime + a secure timestamp **before** `tauri-action` notarizes the app bundle — otherwise the notary service rejects the unsigned nested binary. Because the helper compiles only on a macOS 26 / Xcode 26 toolchain while the app bundle builds on the proven macOS 15 image, a **dedicated `build-apple-fm-helper` job** (`runs-on: macos-26`, in both `release-desktop.yml` and the signed `release-candidate.yml` build) compiles + signs it and uploads it as an artifact; the Developer ID is provisioned into an isolated keychain via `scripts/ci/apple-fm-signing-keychain.sh` (added to the search list so `codesign` resolves the identity by name — safe in this dedicated job, which runs no `tauri-action`, and torn down at the end). The arm64 bundle build downloads that artifact and copies the already-signed binary in (`GLASSBOX_PREBUILT_APPLE_FM_HELPER`); its embedded signature survives, so the bundle's notarization covers it. The Intel bundle omits the helper (arm64-only). The job no-ops signing on secret-less runs.
