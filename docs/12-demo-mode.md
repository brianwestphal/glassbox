...
- Seven demo scenarios shall be available:
  ...
  7. AI review notes inline with the diff — the `src/auth/session.ts` demo file carries an illustrative set of AI-authored review notes (rationale / proof-with-artifact / risk / an outdated note) served via `demoReviewNotes` (doc 20), plus the single human reply threaded beneath the line-31 risk note, so a screenshot captures the review-comment-style rendering and the threading loop.
