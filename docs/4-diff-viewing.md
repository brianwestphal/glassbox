...
- **A / B single-side focus** — Show only one side at a time: **A** displays just the old image, **B** just the new, each as a single-image viewer. These segments sit between Metadata and Side by Side and let a reviewer focus on, zoom into, and annotate one image without the other competing for space. See [28. Single-Side Image Focus](28-image-side-focus.md) for the full requirements.
