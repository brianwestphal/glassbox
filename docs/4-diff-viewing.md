...
- **Scope filter chips** — A segmented control on the diff toolbar (`All` / `Adds` / `Removes` / `Changed`) shall let the user hide diff rows that aren't relevant to what they're inspecting. Implemented as a CSS class on `#diff-container` (`scope-adds` / `scope-removes` / `scope-changed`) that drives `display: none` on the matching `.diff-line` rows; no server change. The setting is persisted in `user_preferences.scope_filter` and restored on load.
- **Unified mode** — Each `.diff-line` is independent, so the filter directly hides or shows individual rows. `Changed` hides `.context` lines, `Adds` hides `.context` + `.remove`, `Removes` hides `.context` + `.add`.
- **Split mode** — Diff lines are grouped under `.split-row` containers (left + right cell). The filter uses CSS `:has()` to hide an entire `.split-row` whose children don't satisfy the filter (e.g. under `Adds`, a row is hidden if it contains no `.diff-line.add`). A modified row whose left cell is `.remove` and right cell is `.add` stays visible under `Adds` because the row *contains* an add — by design, so the user still sees the line being changed in context. The wholesale `.split-row` hide keeps the gutter alignment clean.
