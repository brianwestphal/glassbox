...
- The delete action shall be scoped to backups. Only a `reviews.bak-<timestamp>` sibling may be removed; the live cluster, a staged `reviews.migrating-<timestamp>` directory, and the quarantined `reviews.unreadable-<timestamp>` directories from §9.5 shall all be refused. The quarantined directories are deliberately excluded rather than merely unlisted: one can be the user's *only* copy of data that never migrated, so a one-click delete for it would be a materially riskier feature.

Implemented by `src/db/migrate-major.ts` (`migrateMajorIfNeeded`) over the `pglite-migrate` library, called from `getDb()`; the backup listing/deletion by `src/db/backups.ts` behind `GET /api/db-backups` and `DELETE /api/db-backups/:name`, rendered by `src/client/settings/generalTab.tsx`. Covered by mocked unit tests plus a live PG17→PG18 round-trip (`npm run test:live`) and an e2e over the settings section.
