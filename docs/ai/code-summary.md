...
- `check:features` — feature/requirement coverage report (`scripts/check-features.ts`), the axis orthogonal to line coverage. Extracts a requirement unit per documented behavior from `docs/[0-9]*.md` (both `**FR-N.M**`/`**NFR-N.M**` ids and older `### N.M` subsections) and flags any unit with no asserting test in `docs/testing/feature-coverage.json` — including stateful units that list no state transitions. Advisory (`-- --strict` to fail on gaps); map ships partial. See `docs/testing/9-feature-coverage.md`; structural invariants in `tests/unit/conventions.test.ts`.
