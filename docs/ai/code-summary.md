...
| `difftool-launch.ts` | Pure, unit-tested decision logic for the `glassbox-difftool` wrapper: `planSnapshot()` (dir-diff vs per-file dereference layout, preserving the filename in per-file mode) and `resolveLaunchTarget()` (desktop bundle → launcher shim, else npm → sibling `cli.js`). Exports `DIFFTOOL_BLOCK_ENV` (the `GLASSBOX_DIFFTOOL_BLOCK` contract with the macOS launcher shim). |
