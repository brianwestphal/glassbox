...
| Work on the image lightbox / note-artifact regions | `src/client/lightbox.tsx` (shared full-screen lightbox + region draw), `src/client/diff/noteArtifactRegions.tsx` (artifact click → lightbox → pending region), `src/components/reviewNoteRegionThumb.tsx` (the marked-region thumbnail on a reply). Region carried into the reply via `createAnnotation`'s `region` (→ `region_data` with an `artifact` uri). Doc 25 / GB-953. |
