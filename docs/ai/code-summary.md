...
| `settings.ts` | **Plugin preferences** (doc 29 FR-29.12, GB-1047): per-plugin preference values. `readPluginSetting` / `writePluginSetting` (global config or project settings per the manifest pref's `scope`, default fallback), `readPluginPreferenceValues` (all declared prefs, for the UI). Backs `PluginContext.getSetting`/`setSetting` (`loader.ts` `makeContext`). Non-secret only — keychain-backed secret prefs are GB-1054. |
