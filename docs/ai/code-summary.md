...
| `review-notes/` | AI-authored review notes (doc 20) — P1. `types.ts` (note-kind vocab + constants), `sarif.ts` (note → SARIF 2.1.0 `result` mapping; standard fields + one custom `ext-ai-tool-confidence`), `store.ts` (path-sharded `.pr-notes/notes/<src>.NNNNNN.sarif` layout, 10k-cap roll-over, run-per-(producer,commit) grouping, snippet+fingerprint, lossless read-modify-write; plus guid-keyed `removeNote`/`updateNote`, a dedup `coalesceFile`/`coalesceAll`, and `noteSourceForShardPath`/`notedSourcesInFiles` — map a review's own note shards to their source paths (scoped to shards in the review's diff, NOT every note-bearing path on disk), powering the sidebar note icon + unchanged-file surfacing), `unchanged-files.ts` (`collectNoteOnlyFiles` — files whose note shard is in THIS review's diff but whose source wasn't changed, each surfaced as an `unchanged` all-context `FileDiff`; scoped to the review's own note changes via `noteSourceForShardPath`, not the whole on-disk `.pr-notes` tree; injected at review creation in `cli.ts`, doc 20 §20.6 / GB-1137), `context-notes.ts` (`reanchorNotesForRange` + `renderContextNotes` — notes anchored in an expanded collapsed region, re-anchored/range-scoped/stale-dropped + rendered to rows; served by `/context` + spliced by `hunkExpander`, doc 32 / GB-1139), `origin.ts` (`resolveNoteOrigins` — fills each note's origin-commit subject/full-message from git (`getCommitInfo` in `src/git/repo.ts`, cached per sha) for the provenance label, doc 20 §20.6 / GB-1142; the commit sha itself comes from SARIF `versionControlProvenance`, exposed on `ReviewNoteView.origin` by `loadReviewNotesForFile`), `cli.ts` (`runNoteCli` — `glassbox note add` / `update` / `remove` / `coalesce` / `instructions`), `instructions.ts` (the canonical inbound AI-instructions contract, printed by `glassbox note instructions` for orchestrators to inject — doc 20 §20.4; also instructs the **AI-driven cross-cutting consolidation pass** the generating AI runs after `coalesce`: merge near-duplicate notes + link related notes across files via the existing `update`/`remove` primitives + shared `--ticket`/inline body references — no Glassbox link primitive built until a driver emits structured links), `view.ts` + `loadReviewNotesForFile` (store.ts) — the P2 **reader/render** path: flatten `.pr-notes/` SARIF into diff-anchored view items that `DiffView` server-renders full-width below their line (`ReviewNoteRows`, styled `ai-note-review` + per-kind badge), flow-broken in split like annotations. `reanchor.ts` (`reanchorReviewNotes`) — P3: re-matches each note's authored snippet against the reviewed diff at load (moves shifted notes, flags vanished ones `stale`), mirroring the human stale matcher. `/file/:id` loads + re-anchors them and **filters out stale notes** (GB-1140: outdated notes are hidden, relative to the reviewed content — an older commit still shows them); the context-reveal path filters likewise. Note bodies render as markdown via `src/utils/noteMarkdown.ts` (`renderNoteMarkdown` — safe escape-first **block + inline** renderer: headings, nested lists, fenced code, blockquotes, rules, plus code/bold/italic/links; shared with `client/diff/aiNotes.tsx`). Bodies round-trip through SARIF as `message.markdown` (source) + `message.text` (a plain-text flattening via `src/utils/flattenMarkdown.ts`); the reader takes `markdown ?? text`, so a third-party producer's rich body survives. **Embedded links** (doc 20 §20.6): `--related <file:line>` writes `result.relatedLocations`, and a body's `[text](N)` renders as a jump-to-line link handled by `navigateToLocation` in `client/diff/goToDefinition.tsx` (shares the doc-13 destination handling). **Artifacts** (P4): `--artifact` writes SARIF `result.attachments` (+ sha-256 hash on the attachment; image attachments idempotently add the Git LFS `.gitattributes` filter). `loadReviewNotesForFile` reads text/diagram-source content (path-contained, size-capped, binary-skipping) → collapsible code block; image artifacts (`.png`/`.webp`/…) render as `<img>` served by `GET /api/review-notes/artifact` (path-contained, content-typed). Live diagram rendering of diagram-source artifacts shipped via the doc-29 content plugins (`renderNoteArtifacts` in `src/plugins/artifacts.ts`, wired into `pages.tsx` + `api/context.ts`; falls soft to the code block when no matching plugin is installed). `format.ts` (`reviewNotesPromptSection` / `reviewNotesExportSection`) — P5: folds notes into the AI-analysis prompt (`runAnalysisBatch` in `ai/shared.ts`, informing risk/narrative/guided) and into the `latest-review.md` export (`export/generate.ts`). **Threading**: note rows carry their `guid` (`data-note-id`) + a Reply button; a reply is a human annotation linked by the nullable `annotations.reply_to_note_id` column (migrated in `db/connection.ts`), tagged "↳ reply" in `diffView.tsx` / `annotations/render.tsx` and **nested beneath their note** (`repliesByNote` → `ReviewNoteRows`; orphan replies fall back to line rendering; the shared `AnnotationItem` component renders both). **Stale notes hidden** (GB-1140): outdated notes are dropped from the display (no in-diff "outdated" badge / Keep / Discard — those were removed); a note is removed from `.pr-notes/` producer-side via `glassbox note remove`/`coalesce`. The `removeNote` store op + `DELETE /api/review-notes/:guid` (`src/routes/api/review-notes.ts` + `src/api/review-notes.ts`) remain as an API. Dispatched from `cli.ts` (the `note` subcommand intercepts before normal arg parsing). |
...
| `context.ts` | `GetContextLinesReq/Resp` + `getContextLines()` for hunk expansion. The response also carries optional `notes:[{line,html}]` — server-rendered review-note rows for notes in the revealed range (doc 32 / GB-1139). |
...
| `attachments.ts` | Reviewer file attachments (doc 25): upload / list / list-all / raw / quicklook / delete on an annotation. |
| `review-notes.ts` | AI review-note reader endpoints (doc 20): artifact fetch + `DELETE /review-notes/:guid` (stale-note discard). |
| `export.ts` | `ReviewExportSchema` — the SSOT zod schema for the structured JSON export (`buildReviewExportData`), `.parse()`d before write. No fetch caller; the schema is shared by the export writer + `--on-complete` consumers. |
...
| `api/context.ts` | `/context/:fileId` line-range fetch for hunk expansion. Also returns review notes anchored in the revealed range (doc 32 / GB-1139): loads the file's notes (real `.pr-notes/` or demo), `reanchorNotesForRange` + `renderContextNotes` (`src/review-notes/context-notes.ts`) re-anchor/range-scope/drop-stale + render, and the client (`hunkExpander.tsx`) splices the HTML in. |
| `api/project-settings.ts` | `.glassbox/settings.json` read/write (per-repo `appName`), via the shared `project-settings-store.ts`. |
| `api/plugins.ts` | Content-plugin management (doc 29, GB-1040): `GET /plugins` (list + state), `GET /plugins/ui` (registered UI elements of enabled plugins, doc 30/GB-1058), `GET /plugins/available` + `POST /plugins/:id/install-bundled` (opt-in install with readiness + guided setup, FR-29.20/GB-1069 — returns the `InstallResult` + refreshed installed/available lists), `POST /plugins/:id/disabled` (global\|project toggle), `POST /plugins/install` (from a path), `POST /plugins/:id/preferences` (GB-1047), `POST /plugins/:id/action` (config-layout button *or* UI-element → plugin `onAction`, GB-1059/GB-1058; response carries the refreshed list + the optional `result` message), `DELETE /plugins/:id`. Each mutation reloads the subsystem (except `action`, which runs against the live plugin) and returns the refreshed list — including each plugin's `configLayout` + resolved `configLabels`. |
...
| `imageDiff.tsx` | Image diff view (metadata / **A** / **B** / **side-by-side** / difference / slice modes; docs 24 + 28). The **A**/**B** panels (`data-panel="a"`/`"b"`, gated on a two-sided change) are single-image focus views between Metadata and Side by Side — A shows only the old image, B only the new — each its own zoom canvas + image + a `data-region-side`-tagged overlay (doc 28). Side-by-side is the default for a two-sided change: two panes (`[data-sxs-pane="old"|"new"]`), each its own zoom canvas + image + region overlay, with `data-sxs-orientation` on the panel flipping left-right↔over-under. Each visual panel's zoom-wrap carries a `[data-region-overlay]` for image-feedback region boxes (the side-by-side overlays add `data-region-side` so A-only/B-only regions render only on the matching pane), and the view ends with a `[data-image-feedback]` mount point the client populates (doc 23). Each SxS pane + the single-image viewer also carry a hover `.image-expand-btn` (`data-image-src`/`data-image-alt`) that opens that side in the shared lightbox (doc 24 FR-24.9 / GB-966); `diff/index.tsx` delegates the click → `openLightbox` (the blend difference/slice canvases get no button). |
| `reviewHistory.tsx` | Review history page table. |
| `reviewNoteRegionThumb.tsx` | The marked-region thumbnail(s) rendered on a note reply (doc 25 / GB-953, GB-959) — one or more reviewer-drawn rectangles on an AI review-note image artifact, decoded/grouped via `src/utils/artifactRegions.ts`. |

### `src/db/` — database layer (raw SQL, no ORM)

| File | Purpose |
|------|---------|
| `connection.ts` | PGLite init, schema apply, major-version upgrade, corrupt-DB recovery, `addColumnIfMissing` migrations. Exported `setDataDir`, `getDb`. Pins `database: 'template1'` in the `PGlite` constructor so on-disk data dirs created under PGLite ≤0.3.x (whose default working DB was `template1`) keep opening after the 0.4.x upgrade (whose default is `postgres`). On an open failure `getDb` consults `migrate-major.ts` **before** the corrupt-DB recovery, since a PG-major mismatch and real corruption are indistinguishable by message. |
| `migrate-major.ts` | One-time PostgreSQL **major-version** migration (doc 9 §9.1a), over the `pglite-migrate` library. PGLite embeds a fixed major and has no `pg_upgrade`, so a PG17 dir cannot be opened by the PG18 engine at all. `migrateMajorIfNeeded(dbPath, initSchema)` reads `PG_VERSION` without booting the cluster, downloads the pinned old engine **on demand** (`acquireEngine`, `cache: 'ephemeral'` — not shipped, ~25 MB), copies rows into a **staged sibling** built by the app's own `initSchema`, validates at the `'full'` level, then `swapIntoPlace`. A verified backup is taken **before** the old engine opens the dir (opening can write during recovery); `keepOld: false` drops the swap's redundant second copy. Every failure returns `blocked` with the canonical dir untouched, which `getDb` turns into a start-up error rather than a quarantine — quarantining would strand the data permanently. |
| `backups.ts` | Retained pre-upgrade database backups (doc 9 §9.1a). `listDatabaseBackups(dbPath)` finds `reviews.bak-<stamp>` siblings with their recursive size + parsed timestamp; `deleteDatabaseBackup(dbPath, name)` removes one. Deliberately scoped to the `.bak-` prefix: it must NOT match the `reviews.unreadable-<stamp>` quarantine dirs (§9.5), which can be a user's only copy of data that never migrated. `listQuarantinedDirectories` does the same for the `reviews.unreadable-<stamp>` dirs (§9.5), kept a SEPARATE list so a caller cannot offer them the delete affordance; `resolvePreservedDirectory` validates either prefix for the read-only reveal. Backs `GET /api/db-backups`, `DELETE /api/db-backups/:name`, and `POST /api/db-backups/:name/reveal` (`src/routes/api/db-backups.ts`), rendered as the Settings → General **Database backups** (deletable) and **Preserved unreadable data** (reveal-only) sections. |
| `ddl.ts` | `SCHEMA_CORE_SQL` (reviews, review_files, annotations, **attachments**) and `SCHEMA_AI_SQL` (ai_analyses, ai_file_scores, user_preferences). Single source of truth. |
| `schemas.ts` | Zod schemas for every DB row shape (`Review`, `ReviewFile`, `Annotation`, `AIAnalysis`, `AIFileScore`, `UserPreferences`, `Attachment`) + `parseRow` / `parseRows` / `parseJsonColumn` helpers. The SSOT for runtime row validation; `queries.ts` / `ai-queries.ts` / `attachment-queries.ts` route every PGLite result through these. |
| `queries.ts` | CRUD for reviews, review_files, annotations. |
| `ai-queries.ts` | CRUD for ai_analyses, ai_file_scores, user_preferences. |
| `attachment-queries.ts` | CRUD for the `attachments` table (doc 25); pairs with disk I/O in `src/attachments/store.ts`. |
| `ids.ts` | The shared id generator (`Date.now().toString(36) + Math.random().toString(36).slice(2,10)`). |
...
| File | Purpose |
|------|---------|
| `diff.ts` | `getFileDiffs(mode, cwd)`, `parseDiff()`, mode → `git` argv builder. Also lists untracked files for uncommitted mode (and for `--files` mode, scoped to the requested patterns), walks all files for `--all`, and runs `git diff --no-index` for the `--diff <A> <B>` direct-comparison mode (doc 18) — `directComparisonRoots(mode)` resolves the per-side display roots and `getModeFileContent(mode, file, 'old'\|'new', cwd)` reads file content for context expansion, branching to disk reads (rootA/rootB) for `--diff` and to git refs for every other mode. `diffRawContent(displayPath, oldBuf, newBuf)` produces a `FileDiff` from two raw buffers (no git refs / no caller-managed paths) by running `git diff --no-index` against a throwaway temp pair — used by the doc-19 difftool append endpoint. |
| `spawn.ts` | The low-level git subprocess helpers (`git()` spawn wrapper + error enrichment) and **`scrubbedGitEnv()`** — the env every internal git subprocess runs with, with `git difftool`'s leaked `GIT_EXTERNAL_DIFF` / `GIT_DIFF_PATH_COUNTER` / `GIT_DIFF_PATH_TOTAL` stripped (used by `repo.ts`, `diff.ts`, and `image.ts`'s `git show`). Left in place those would make Glassbox's own `git diff`/`git show` re-invoke the difftool helper → recursion + empty diff ("No changes found"). See doc 19 NFR-19.12. (`scrubbedGitEnv` historically lived in `repo.ts` and is still re-exported from there for back-compat.) |
| `repo.ts` | `getRepoRoot`, `getRepoName`, `getHeadCommit`, `isGitRepo`. Re-exports `scrubbedGitEnv()` from `spawn.ts` (its current home). |
| `gitignore.ts` | Automatic `.gitignore` management (doc 27). Pure `computeGitignore(existing)` keeps the canonical `/.glassbox/*` + `!/.glassbox/settings.json` block (idempotent, replaces stale `.glassbox` lines, opt-out via a commented rule); `ensureGlassboxGitignored(glassboxDir)` gates on a real `.glassbox` dir inside a git repo and writes. Called from `cli.ts` at launch. |
| `types.ts` | `ReviewMode` (incl. the doc-18 `diff` + doc-26 `ground-truth` variants), `GroundTruthEntry`, `FileDiff`, `DiffHunk`, `DiffLine`. |
| `../ground-truth/manifest.ts` | doc 26 — `loadGroundTruthManifest(path)`: read/validate (`version:1` `comparisons[]`, or `version:2` adding optional ordered `sets[].steps[]` — P3a), resolve actual/expected paths relative to the manifest dir, image-only check, dedup keys. v2 flattens each step into one `GroundTruthEntry` keyed `set:<i>/<j>-<basename>` carrying `setIndex`/`setLabel`/`stepIndex`/`stepCount` (steps inherit the set's `expectedKind`). |
| `../ground-truth/perceptual-diff.ts` | doc 26 P2 — `comparePerceptual(actual, expected, pluginDecode?)` (**async**): decode PNG/JPEG to RGBA (`pngjs`/`jpeg-js`) + `pixelmatch` (YIQ, threshold 0.1, AA-tolerant) → difference score (fraction of changed pixels); `dimension-mismatch`→1. When core can't decode a format, falls back to an installed **image-decoder plugin** (`decodeImageWithPlugin`, doc 29 FR-29.19 `imageDecoders`, GB-1063) before returning `undecodable` (SVG/corrupt/no-plugin → `undecodable`). Score computed at launch + stored on `review_files.difference_score`; the ground-truth launch branch inits plugins before scoring. Format helpers in `src/utils/diffScore.ts`. |
| `../ground-truth/promote.ts` | doc 26 P3d — `promoteGroundTruthBaselines(manifestPath)`: optional baseline rotation. Copies **only** `expectedKind: "previous-actual"` entries' actual→expected (specs/references never overwritten); dispatched as the `glassbox ground-truth promote <manifest>` subcommand in `cli.ts`. No server, no review DB; Glassbox owns no baseline state. |
| `../ground-truth/presentation.ts` | doc 26 §26.1/§26.3 polish — pure helpers shared by the diff page + `/files` route. `groundTruthSideLabels(mode)` → "Expected (A)" / "Actual (B)" pane captions (via `ImageDiff.sideLabels` / `DiffView.imageSideLabels`); `groundTruthMetaByFileId(mode, files)` → `fileId → {label, expectedKind, setIndex?, setLabel?, stepIndex?, stepCount?}` map the `/files` response carries for ground-truth reviews (drives the flat named-comparison sidebar list + set groups, `groundTruthListJsx`); `groundTruthStepNav(mode, fileId, files)` → diff-header "Step k of N" navigator data (set label + prev/next sibling step file-ids, P3b); `EXPECTED_KIND_LABELS` (Spec / Reference / Baseline). |
| `image.ts` | Image side retrieval from git (old/new), binary detection, format identification. |
| `image-blobs.ts` | On-disk blob store for image/SVG bytes that have no git ref / working-tree source — a difftool review (doc 19, GB-863) and demo SVGs (GB-947). Content under `<dataDir>/image-blobs/`, keyed `fileId`+side, cleared on session start + teardown; read by the `/image` route as a fallback. |
...
| `parseDiff.ts` | Pure text parser for raw unified `git diff` output → `FileDiff[]` with typed hunks/lines (`parseDiff`, split out of `diff.ts`; re-exported from it). No git/fs. |
...
| File | Purpose |
|------|---------|
| `generate.ts` | Generates `.glassbox/latest-review.md` (overwritten) and `review-{id}.md` (archived). Includes category summary table, "Items to Remember" block, grouped-by-file annotations, and "Instructions for AI Tools" section. |
| `auto-export.ts` | Debounced (2s) regeneration triggered by annotation writes. Fired immediately on review completion. |
| `build-data.ts` | `buildReviewExportData()` — pure builder for the structured JSON export (`ReviewExportSchema` in `src/api/export.ts`); annotations grouped by comparison, with ground-truth context + region geometry + attachments. |
| `build-markdown.ts` | `buildReviewMarkdown()` — pure markdown assembly for `latest-review.md` (header/summary/remember/per-file/notes/instructions sections), mirroring `build-data.ts` for the JSON side; `generate.ts` is just data collection + IO. |
| `on-complete-hook.ts` | `runOnCompleteHook()` — runs the `--on-complete <command>` user shell command after explicit completion + export (env-passed paths; never breaks completion). See §11. |
...
### `src/plugins/` — content-plugin subsystem (doc 29, GB-1038)

Opt-in renderers/differs for specialized content types, loaded from
`~/.glassbox/plugins/`. In-process, server-side, at app trust level (opt-in
install = the trust boundary; no sandbox). Gated by `PLUGINS_ENABLED`
(`feature-flags.ts`). Modeled on Hot Sheet's `src/plugins/`.

| File | Purpose |
|------|---------|
| `types.ts` | The contract: `ContentMatch` (ext/MIME/sniff), `RenderInput` / `DiffInput`, `RenderedView` (`svg` \| `html`), `ContentRenderer` / `ContentDiffer`, `ImageDecoder` + `DecodedImage` (doc 29 FR-29.19 `imageDecoders`, bytes→RGBA), `PluginRegistration` (`renderers?`/`differs?`/`imageDecoders?`/`reviewHooks?`), `PluginContext`, `ContentPlugin` (`activate(context) → registration`). Pure types; a standalone copy ships in the dev guide (GB-1041) so third-party plugins don't depend on the Glassbox package. |
| `manifest.ts` | `PluginManifestSchema` (zod `.loose()`) + `parseManifest(raw)` — the single trust boundary; returns `null` (never throws) on an invalid manifest. |
| `registry.ts` | `ContentPluginRegistry` (in-memory) + the dispatcher: `matchSpecificity` (sniff 3 > mime 2 > ext 1), `findRenderer` / `findDiffer` / `findImageDecoder` pick the highest `(priority, specificity)` match (`addImageDecoders`/`imageDecoderCount` for the doc 29 FR-29.19 capability). Pure. |
| `svgSize.ts` | `ensureIntrinsicSvgSize(svg)` — pure normalizer applied to every plugin SVG in `renderContent`: injects absolute `width`/`height` from the `viewBox` when the root has none or a percentage (Mermaid emits `width="100%"`, which has no intrinsic size in an `<img>` and collapsed to ~0×0 in both render paths). |
| `hooks.ts` | `notifyReviewCreated` / `notifyReviewCompleted` — fail-soft review lifecycle hook dispatch (doc 31), split from `index.ts` (re-exported there for call-site stability). |
...
| `index.ts` | Public API + process-global registry: `initContentPlugins()` (startup load, idempotent, fail-soft), `renderContent` / `diffContent` (return `null` on disabled/no-match/throw → caller falls back), `decodeImageWithPlugin(bytes, path, mime?)` (doc 29 FR-29.19 `imageDecoders` — bytes→RGBA via the best-matching decoder, `null` fail-soft; consumed by the perceptual diff), `runPluginAction` (config/UI action → `onAction`), `listPluginUIElements` (doc 30), `notifyReviewCreated` / `notifyReviewCompleted` (doc 31 lifecycle hooks — fail-soft dispatch to loaded plugins' `reviewHooks`; `onReviewCreated` fired from `server.ts` after plugin load, `onReviewCompleted` from the complete route), `getLoadedPlugins`, `pluginsEnabled`, and `__setContentRegistryForTest` / `__resetContentPluginsForTest` seams. |
...
| `fileView.ts` | **File-diff-viewer** integration (doc 29 FR-29.2, GB-1042 + GB-1052). `pluginRendersFile(path)` — the gate (cheap ext/MIME pre-check). `renderPluginSvgSide(mode, filePath, oldPath, side, cwd)` — renders one side of a file to SVG via a plugin. A plugin-rendered file (e.g. `.dot`) is treated **like an SVG file**: the image route serves `renderPluginSvgSide` output as `image/svg+xml`, the `/file/:id` `view=rendered` branch builds `<ImageDiff data-is-svg>`, and `/files` flags it (`pluginRendered`) so the client shows the **Code \| Rendered** toggle — Code = the source text diff, Rendered = the plugin SVG in the image viewer (zoom + A/B/difference/side-by-side/slice, all reused). |
| `install.ts` | **Desktop delivery** (doc 29 FR-29.7, GB-1039): `installBundledPlugins()` (run from `initContentPlugins` before discovery) seeds `~/.glassbox/plugins/` from the bundled plugins (`bundledPluginsDir()` → `server/plugins` in prod, `dist/plugins` in dev; overridable via `GLASSBOX_BUNDLED_PLUGINS_DIR`) with a version + content-hash freshness check (`compareVersions` / `hashPluginDir` / `shouldInstall`) and a `dismissed-plugins.json` dismiss-list so uninstalls stick. Skips a plugin whose manifest sets **`autoInstall: false`** (separately-installable, e.g. PlantUML — GB-1046). `discoverBundledPlugins()` lists bundled plugins (shared by the available list + install action). `installPluginFromDisk` (symlink + un-dismiss) / `uninstallPlugin` (remove + dismiss) / `undismissPlugin` back the management UI (GB-1040). Fail-soft. |
| `readiness.ts` | **Opt-in-install readiness checks** (doc 29 FR-29.20, GB-1069): `checkRequirement`/`checkRequirements` probe a manifest requirement by running `command <checkArgs>` (injectable `CommandProbe`); `requirementMet` looks one up. A requirement is something the host can only detect + instruct on (a JRE, `npm`), not install. |
| `available.ts` | **Available-to-install list** (doc 29 FR-29.20, GB-1069): `listAvailablePlugins()` = opt-in (`autoInstall:false`) bundled plugins not yet installed, each with a readiness report + provision notes + a `selfContained` flag; `describeAvailablePlugin` builds one; `bundledManifestById` resolves a bundled manifest. |
| `install-action.ts` | **The UI install action** (doc 29 FR-29.20, GB-1069): `installAvailablePlugin(id)` copies the bundle → `~/.glassbox/plugins/`, checks readiness, auto-runs the provisioning it can (`fetch` a file always; `npm-install` only if `npm` present), and returns a structured `InstallResult` (`status: ready`\|`needs-setup`\|`error`, `requirements`, `provisioned`, `instructions`). Every external effect (spawn/fetch) is injectable; non-destructive re-install preserves provisioned assets. |
| `settings.ts` | **Plugin preferences** (doc 29 FR-29.12, GB-1047 + GB-1054): per-plugin preference values. `readPluginSetting` / `writePluginSetting` (global config or project settings per the manifest pref's `scope`; **secret** prefs → the OS keychain via `src/ai/keychain.ts` `getSecretFromKeychain`/`saveSecretToKeychain`/`deleteSecretFromKeychain`, account `plugin-<id>-<key>`). `readPluginPreferenceValues` (all resolved values, internal) + `readPluginPreferenceDisplay` (UI-safe: **masks secrets**, reports `secretConfigured`). Backs `PluginContext.getSetting`/`setSetting` (`loader.ts` `makeContext`). |
...
| `lfs.ts` | `isLfsPointer` — is this blob a Git LFS pointer rather than real content? An LFS-tracked file is stored as a three-line text pointer, so git diffs it as text (no `Binary files` header) and `git show` returns the pointer. Used by `parseDiff` (flag binary, drop the hunks), `git/image.ts` (retry through `git cat-file --filters`, else fail soft), and the review-note artifact paths. |
...
| `parseBody.ts` | `parseBody(c, ReqSchema)` (zod-validate an incoming JSON body → structured 400 on failure) + `requirePathParam(c, name)` (non-empty path-param guard) + `requireSlugParam(c, name)` (adds the `SAFE_SLUG_RE` filesystem-safety check for ids that reach the fs — plugin/theme routes, doc 14) + `errorResponse(c, msg, status)` (the single structured-error idiom every route uses). The server-side boundary validators paired with the client-side `apiCall(RespSchema, …)`. |
| `safeSlug.ts` | `SAFE_SLUG_RE` — the filesystem-safe id pattern (no path separators / leading dot) shared by `requireSlugParam` and the plugin-manifest `id` schema. |
| `compareVersions.ts` | `compareVersions(a, b)` — the single dotted-version comparator (update check, plugin installer freshness, channel-api minimum-Claude-version). |
| `openOS.ts` | `openOS(target, mode)` — cross-platform OS-open shell-out (`reveal` in file manager / `edit` in default app / `quicklook` preview). Argv-array spawn, never shell interpolation. Backs file reveal (doc 21), attachment Quick Look (doc 25), and open-external. |
| `mime.ts` | MIME-type helpers (extension → content type) used by the attachment + image-artifact routes. |
| `diffScore.ts` | Format helpers for the ground-truth perceptual difference score (doc 26 P2). |
| `artifactRegions.ts` | Pure decode/group of a `region_data` JSON array into per-artifact region groups (doc 25 / GB-959). |
| `noteMarkdown.ts` | `renderNoteMarkdown(body, related?)` — safe, escape-first markdown renderer shared by AI review notes + the risk/narrative/guided notes. **Blocks**: paragraphs, ATX headings (rendered `h4`–`h6`), nested unordered/ordered lists, fenced code, blockquotes, thematic breaks. **Inline**: code, bold, italic, scheme-gated links, and SARIF **embedded links** (an integer destination indexes `related` → a no-href `.ai-note-loclink` the client navigates from). Escape-first + fixed tag allowlist + depth-capped recursion, so the output is safe for `raw()` by construction. Block patterns must match *escaped* text (a blockquote marker is `&gt;`). |
| `flattenMarkdown.ts` | `flattenMarkdown` — markdown → plain text for SARIF `message.text`, the readable fallback beside the `markdown` source (doc 20 §20.2). Drops heading/emphasis/fence markers, keeps fence contents and list/quote markers, renders links as `text (url)`. No security role. |
...
| `stores/index.ts` | Reactive state via kerfjs `defineStore`: `reviewStore` (reviewId, currentFileId, files, fileOrder, annotationCounts, staleCounts, filterText, `pluginRendered`, `notedFileIds` — ids of files with AI review notes, doc 20 §20.6), `diffViewStore` (diffMode, wrap, whitespace, image mode, svg mode, highlight, collapsedFolders), `aiStore` (sortMode, scores, analysis states, fileNotes, guidedNotes, configured/enabled flags), `dragStore`. Computed `filteredFiles`, `aiEnabled`. `getAnalysisModeState(mode)` helper. |
...
| `share.tsx` | OS share sheet (`navigator.share`) with clipboard fallback. |
| `lightbox.tsx` | Shared full-screen image lightbox (zoom/pan up to 8×, optional region-draw) used by image side full-screen (doc 24 FR-24.9), attachment image preview (doc 25), and note-artifact region marking (GB-953/GB-959). Pure zoom/pan math in `lightboxZoom.ts`. |
| `lightboxZoom.ts` | DOM-free zoom/pan math for the lightbox (GB-963). |
| `aiError.ts` | Client-side AI-error message formatting/surfacing. |
| `timing.ts` | Small client timing helpers (debounce/throttle-style utilities). |
...
`DELETE /annotations/:id`, `PATCH /annotations/:id/region`,
`PATCH /annotations/:id/move`, `POST /annotations/:id/keep`,
`POST /annotations/stale/delete-all`, `POST /annotations/stale/keep-all`,
`GET /annotations/all`.

Attachments (doc 25): `GET /attachments/all`,
`GET|POST /annotations/:id/attachments`, `GET /attachments/:id/raw`,
`POST /attachments/:id/quicklook`, `DELETE /attachments/:id`.

Review notes (doc 20): `GET /review-notes/artifact`,
...
See `src/db/ddl.ts` for the authoritative SQL. Quick reference:
...
- `user_preferences(id='singleton', sort_mode, risk_sort_dimension, show_risk_scores, ignore_whitespace, svg_view_mode, last_image_mode, image_sxs_orientation)` — one row (`last_image_mode` defaults to `side-by-side`, also accepts `metadata`/`a`/`b`/`difference`/`slice` — `a`/`b` are the single-side focus modes, doc 28; `image_sxs_orientation` is `left-right`|`over-under`, doc 24)
...
`idx_annotations_file`, `idx_attachments_annotation`,
`idx_ai_analyses_review`, `idx_ai_file_scores_analysis`.
...
- `demo:capture-stills` — regenerate the static README screenshots (`assets/demo-{guided-review,risk-mode,narrative-mode,annotations,settings,review-notes,image-comparison,ground-truth,plugin-rendered,plugin-mermaid-note}.png`, plus a stand-alone `.svg` for non-`pngOnly` scenes). Boots each `--demo:N` (or `--ground-truth`) scenario in turn, runs a minimal scenario-specific UI setup (open the showcased file, flip a sort mode, open the settings dialog, …), and captures both a Playwright PNG and a `domotion-svg` stand-alone SVG per scenario. Lives in `scripts/demo/capture-stills.ts`. Must run outside the command sandbox (Chromium). (No still for demo scenario 6 / `--diff` — its only visible difference is the sidebar label; the scenario itself remains for hands-on demoing.)
- `demo:capture` — regenerate the animated README hero `assets/demo.svg` (+
  `.svgz`). Boots a real `--demo:1` server, drives the CLI-launch → risk-triage →
  ...
  **Transition model (domotion 0.16+, DM-1414): a frame's `transition` is its
  ...
  default doesn't wedge the run. `domotion-svg` is pinned to 0.27.1 and forced to
...
- **`technical-changelog` skill** (`.claude/skills/technical-changelog/SKILL.md` + `scripts/changelog-analysis.mjs`, GB-1072) — generate a one-page, **diff-grounded** technical changelog for a release into `docs/technical-changelog/<base>-<next>.md`. The skill asks for the next version (HEAD is unreleased; package.json holds the last release), runs the analysis script, then reads the real per-file diffs + verifies each "new" claim against the base tree (`git cat-file -e <base>:<file>`) before writing. `changelog-analysis.mjs` is deterministic git analysis (writes nothing): auto-detects the base = newest production `vX.Y.Z` tag reachable from HEAD (pre-releases excluded, warns if a newer exists), buckets the line delta by area with a **product-only** total (src/src-tauri/plugins/tests/scripts vs docs/scaffolding/generated), and surfaces the Glassbox public-surface deltas — new requirements docs, new first-party plugins, new API/route modules, CLI-flag changes (`src/cli.ts`), and dependency changes (with the three-place external-dep reminder). Modeled on `~/Documents/apple-fm`'s equivalent.

`tsup.config.ts` produces:

- `dist/cli.js` — ESM, Node 20 banner (`#!/usr/bin/env node`). External:
  `@electric-sql/pglite`, `hono`, `@hono/node-server`,
  `@modelcontextprotocol/sdk`, `kerfjs`, `apple-fm`.
