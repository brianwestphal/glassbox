...
| `parseBody.ts` | `parseBody(c, ReqSchema)` (zod-validate an incoming JSON body → structured 400 on failure) + `requirePathParam(c, name)` (non-empty path-param guard). The server-side boundary validators paired with the client-side `apiCall(RespSchema, …)`. |
