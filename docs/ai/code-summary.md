...
| File | Purpose |
|------|---------|
| `cli.ts` | Entry point. Parses args, picks review mode, calls git, creates/resumes review, starts server. Also handles the repo-less `--diff` (doc 18) and `--ground-truth <manifest>` (doc 26) launches. |
