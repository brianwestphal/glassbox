...
| `registry.ts` | `ContentPluginRegistry` (in-memory) + the dispatcher: `matchSpecificity` (sniff 3 > mime 2 > ext 1), `findRenderer` / `findDiffer` pick the highest `(priority, specificity)` match. Pure. |
...
| `index.ts` | Public API + process-global registry: `initContentPlugins()` (startup load, idempotent, fail-soft), `renderContent` / `diffContent` (return `null` on disabled/no-match/throw → caller falls back), `runPluginAction` (config/UI action → `onAction`), `listPluginUIElements` (doc 30), `notifyReviewCreated` / `notifyReviewCompleted` (doc 31 lifecycle hooks — fail-soft dispatch to loaded plugins' `reviewHooks`; `onReviewCreated` fired from `server.ts` after plugin load, `onReviewCompleted` from the complete route), `getLoadedPlugins`, `pluginsEnabled`, and `__setContentRegistryForTest` / `__resetContentPluginsForTest` seams. |
...
| `plugins/` | `uiExtensions.tsx` | Plugin main-app UI extensions (doc 30, GB-1058): `initPluginUi`/`refreshPluginUi` fetch `GET /api/plugins/ui` and render each plugin's registered **button**/**link** into the `#plugin-ui-header` / `#plugin-ui-diff-toolbar` / `#plugin-ui-sidebar-footer` slots (reviewShell.tsx). One `delegate` on `document.body` routes clicks → `POST /api/plugins/:id/action` → toast on the `result.message`. Re-renders after a plugin is enabled/disabled/installed/uninstalled (called from `settings/pluginsTab.tsx`). Stateful controls (toggle/switch/segmented-control) declared-but-not-rendered. |
