...
export function renderNarrativeFileList(container: Element) {
  ...
  const unscored = review.files.filter(f => !orderedFileIds.has(f.id));
  ...
  for (const item of filteredOrdered) {
    const file = review.files.find(f => f.id === item.reviewFileId);
    ...
    const el = toElement(
      <div className={`file-item${item.reviewFileId === review.currentFileId ? ' active' : ''}`}
    ...
    reviewStore.actions.pushFileOrder(item.reviewFileId);
  }
