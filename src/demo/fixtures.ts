/**
 * Static fixture data for the demo review scenarios (doc 12). Split out of
 * the demo engine (`src/demo.ts`) so the ~400 lines of canned files, scores,
 * narrative order, and annotations don't crowd the setup logic. Pure data +
 * the one large-SVG generator used to seed it.
 */
import type { FileDiff } from '../git/diff.js';

export function buildLargeMinifiedSvg(nudge: string): string {
  let body = '';
...
export const GUIDED_NOTES: Record<string, { overview: string; lines: Array<{ line: number; content: string }> }> = {
  'src/auth/session.ts': {
...
export const RISK_SCORES: Array<{ path: string; aggregate: number; scores: Record<string, number>; rationale: string; notes: { overview: string; lines: Array<{ line: number; content: string }> } }> = [
  {
    path: 'src/auth/session.ts',
    aggregate: 0.65,
...
export const NARRATIVE_ORDER: Array<{ path: string; position: number; rationale: string; notes: { overview: string; lines: Array<{ line: number; content: string }> } }> = [
...
export const ANNOTATIONS: Array<{ filePath: string; line: number; side: string; category: string; content: string; replyToNoteId?: string }> = [
