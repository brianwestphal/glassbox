...
import {
addReviewFile,
deleteReviewFile, getAnnotationsForFile,
  getReviewFiles, markAnnotationCurrent,   markAnnotationStale, moveAnnotation,
...
export async function updateReviewDiffs(
  ...
  for (const [path, existingFile] of existingByPath) {
    ...
    if (newDiff) {
      const oldDiff: FileDiff = parseDiffData(existingFile.diff_data) ?? emptyFileDiff(existingFile.file_path);
